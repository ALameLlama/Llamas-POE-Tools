...
class Entangler extends Component
{
    ...
    {
        $this->childRecipes = [
            'toxic' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_toxic") ?? false,
            'bloodletter' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_bloodletter") ?? false,
        ];
