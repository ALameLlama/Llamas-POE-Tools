...
class FrostStrider extends Component
{
    public bool $owned = false;
    public bool $childOwned = false;

    public string $name = 'frost-strider';
    ...
    {
        $this->childRecipes = [
            'frostweaver' => Storage::disk('local')->get("{$this->parent}_{$this->name}_frostweaver") ?? false,
            'hasted' => Storage::disk('local')->get("{$this->parent}_{$this->name}_hasted") ?? false,
        ];
    ...
    {
        return view('livewire.archnemesis.frost-strider');
    }
}
