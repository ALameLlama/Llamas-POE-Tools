...
class TukohamaTouched extends Component
{
    public bool $owned = false;
    public bool $childOwned = false;

    public string $name = 'tukohama-touched';
    ...
    {
        $this->childRecipes = [
            'executioner' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_executioner") ?? false,
            'magma-barrier' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_magma-barrier") ?? false,
            'bonebreaker' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_bonebreaker") ?? false,
        ];
    ...
    {
        return view('livewire.archnemesis.tukohama-touched');
    }
}
