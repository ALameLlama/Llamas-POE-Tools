...
class SoulEater extends Component
{
    ...
    {
        $this->childRecipes = [
            'necromancer' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_necromancer") ?? false,
            'soul-conduit' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_soul-conduit") ?? false,
            'gargantuan' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_gargantuan") ?? false,
        ];
    ...
    {
        return view('livewire.archnemesis.soul-eater');
    }
}
