...
class MagmaBarrier extends Component
{
    ...
    {
        $this->childRecipes = [
            'incendiary' => Storage::disk('local')->get("{$this->parent}_{$this->name}_incendiary") ?? false,
            'bonebreaker' => Storage::disk('local')->get("{$this->parent}_{$this->name}_bonebreaker") ?? false,
        ];
