...
class Invulnerable extends Component
{
    ...
    private function setChildrenRecipes()
    {
        $this->childRecipes = [
            ...
            'juggernaut' => Storage::disk('local')->get("{$this->parent}_{$this->name}_juggernaut") ?? false,
            'consecrator' => Storage::disk('local')->get("{$this->parent}_{$this->name}_consecrator") ?? false,
        ];
