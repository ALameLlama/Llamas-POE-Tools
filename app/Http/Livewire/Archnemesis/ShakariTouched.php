...
class ShakariTouched extends Component
{
    ...
    {
        $this->childRecipes = [
            'soul-eater' => Storage::disk('local')->get("{$this->parent}_{$this->name}_soul-eater") ?? false,
            'entangler' => Storage::disk('local')->get("{$this->parent}_{$this->name}_entangler") ?? false,
            'drought-bringer' => Storage::disk('local')->get("{$this->parent}_{$this->name}_drought-bringer") ?? false,
        ];
