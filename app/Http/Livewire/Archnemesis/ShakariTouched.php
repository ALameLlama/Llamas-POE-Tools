...
class ShakariTouched extends Component
{
    public bool $owned = false;
    public bool $childOwned = false;

    public string $name = 'shakari-touched';
    ...
    {
        $this->childRecipes = [
            'soul-eater' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_soul-eater") ?? false,
            'entangler' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_entangler") ?? false,
            'drought-bringer' => Storage::disk('local')->get(Auth::id() . '/' . "{$this->parent}_{$this->name}_drought-bringer") ?? false,
        ];
    ...
    {
        return view('livewire.archnemesis.shakari-touched');
    }
}
