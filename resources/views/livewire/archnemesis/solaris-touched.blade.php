<div x-data={show:true}>
    <div class="divide-y divide-gray-200 {{ $owned ? 'bg-green-400 dark:bg-green-600' : ($childOwned ? 'bg-yellow-400 dark:bg-orange-400' : 'bg-red-400 dark:bg-red-500')}} border">
        <div class="relative flex items-center justify-between mr-2 ml-2 h-8">
            ...
            <div class="text">
                <div class="flex items-center">
                    ...
                    <label for="solaris-touched" class="font-medium text-gray-700">Solaris-touched</label>
                </div>
            </div>
            <div class="">
                <input wire:click="toggle"
                       id="solaris-touched"
                       aria-describedby="solaris-touched-description"
                       name="recipe"
                       type="checkbox"
        ...
        <div x-show="show" class="ml-4 border-l-2" x-collapse>
            @livewire('archnemesis.empowering-minions', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.invulnerable', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.magma-barrier', ['parent' => $buildParent, 'directParent' => $name])
        </div>
    </div>
</div>
