<div x-data={show:true}>
    <div class="divide-y divide-gray-200 {{ $owned ? 'bg-green-400 dark:bg-green-600' : ($childOwned ? 'bg-yellow-400 dark:bg-orange-400' : 'bg-red-400 dark:bg-red-500')}} border">
        <div class="relative flex items-center justify-between mr-2 ml-2 h-8">
            ...
            <div class="text">
                <div class="flex items-center">
                    ...
                    <label for="empowered-elements" class="font-medium text-gray-700">Empowered elements</label>
                </div>
            </div>
            <div class="">
                <input wire:click="toggle"
                       id="empowered-elements"
                       aria-describedby="empowered-elements-description"
                       name="recipe"
                       type="checkbox"
        ...
        <div x-show="show" class="ml-4 border-l-2" x-collapse>
            @livewire('archnemesis.evocationist', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.steel-infused', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.chaosweaver', ['parent' => $buildParent, 'directParent' => $name])
        </div>
    </div>
</div>
