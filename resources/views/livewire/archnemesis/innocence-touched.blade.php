<div x-data={show:true} class="border">
    <div class="divide-y divide-gray-200 {{ $owned ? 'bg-green-400 dark:bg-green-600' : ($childOwned ? 'bg-yellow-400 dark:bg-orange-400' : 'bg-red-400 dark:bg-red-500')}}">
        <div class="relative flex items-center justify-between mr-2 ml-2 h-8">
            ...
            <div class="text">
                <div class="flex items-center">
                    ...
                    <label for="innocence-touched" class="font-medium text-gray-700">Innocence-touched</label>
                </div>
            </div>
            <div class="">
                <input wire:click="toggle"
                       id="innocence-touched"
                       aria-describedby="innocence-touched-description"
                       name="recipe"
                       type="checkbox"
        ...
        <div x-show="show" class="ml-4 border-l-2" x-collapse>
            @livewire('archnemesis.lunaris-touched', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.solaris-touched', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.mirror-image', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.mana-siphoner', ['parent' => $buildParent, 'directParent' => $name])
        </div>
    </div>
</div>
