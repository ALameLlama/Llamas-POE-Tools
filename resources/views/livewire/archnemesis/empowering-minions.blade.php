<div x-data={show:true}>
    <div class="divide-y divide-gray-200 {{ $owned ? 'bg-green-400 dark:bg-green-600' : ($childOwned ? 'bg-yellow-400 dark:bg-orange-400' : 'bg-red-400 dark:bg-red-500')}} border">
        <div class="relative flex items-center justify-between mr-2 ml-2 h-8">
            ...
            <div class="text">
                <div class="flex items-center">
                    ...
                    <label for="empowering-minions" class="font-medium text-gray-700">Empowering minions</label>
                </div>
            </div>
            <div class="">
                <input wire:click="toggle"
                       id="empowering-minions"
                       aria-describedby="empowering-minions-description"
                       name="recipe"
                       type="checkbox"
        ...
        <div x-show="show" class="ml-4 border-l-2" x-collapse>
            @livewire('archnemesis.necromancer', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.executioner', ['parent' => $buildParent, 'directParent' => $name])
            @livewire('archnemesis.gargantuan', ['parent' => $buildParent, 'directParent' => $name])
        </div>
    </div>
</div>
