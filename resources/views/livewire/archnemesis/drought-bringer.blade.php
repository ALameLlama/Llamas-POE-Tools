<div x-data={show:true} class="border">
    <div class="divide-y divide-gray-200 {{ $owned ? 'bg-green-400 dark:bg-green-600' : ($childOwned ? 'bg-yellow-400 dark:bg-orange-400' : 'bg-red-400 dark:bg-red-500')}}">
        <div class="relative flex items-center justify-between mr-2 ml-2 h-8">
            ...
            <div class="text">
                <div class="flex items-center">
                    ...
                    <label for="drought-bringer" class="font-medium text-gray-700">Drought bringer</label>
                </div>
            </div>
            <div class="">
                <input wire:click="toggle"
                       id="drought-bringer"
                       aria-describedby="drought-bringer-description"
                       name="recipe"
                       type="checkbox"
        ...
        <div x-show="show" class="ml-4 border-l-2" x-collapse>
            @livewire('archnemesis.malediction', ['parent' => $parent])
            @livewire('archnemesis.deadeye', ['parent' => $parent])
        </div>
    </div>
</div>
